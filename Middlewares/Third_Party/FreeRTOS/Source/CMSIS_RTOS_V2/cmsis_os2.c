...
osStatus_t osTimerStart(osTimerId_t timer_id, uint32_t ticks)
{
  ...
  if (IS_IRQ())
  {
    ...
    if ( xTimerChangePeriodFromISR(hTimer, ticks, (BaseType_t* const )&pxHigherPriorityTaskWoken) == pdPASS)
    {
      stat = osOK;
    }
    else
...
osStatus_t osTimerReset(osTimerId_t timer_id)
{
  ...
  if (IS_IRQ())
  {
    ...
    if (xTimerResetFromISR(hTimer, (BaseType_t* const )&pxHigherPriorityTaskWoken ) == pdPASS)
    {
      stat = osOK;
    }
    else
